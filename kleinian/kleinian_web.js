...
* @param {number} iters
*/
export function draw(ctx, width, height, re1, im1, re2, im2, typ, iters) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passStringToWasm0(typ, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        wasm.draw(retptr, addBorrowedObject(ctx), width, height, re1, im1, re2, im2, ptr0, len0, iters);
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('kleinian_web_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
